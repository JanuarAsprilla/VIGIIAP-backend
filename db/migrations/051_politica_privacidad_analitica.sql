-- ─── Política de Privacidad: menciona la analítica de uso anónima ──────────
-- Actualiza el valor SOLO si sigue siendo exactamente el texto por defecto
-- sembrado en 032_politica_privacidad_default.sql -- si un super_admin ya
-- editó el contenido desde el panel de Configuración, este UPDATE no
-- coincide y no toca nada (evita pisar una personalización real de IIAP).
...
WHERE clave = 'politicaPrivacidad'
  AND valor = $$En cumplimiento de la Ley 1581 de 2012 y el Decreto 1377 de 2013 de la República de Colombia, el IIAP informa lo siguiente sobre el tratamiento de sus datos personales:
...
-- También actualiza el fallback en código (src/pages/recursos/PoliticaPrivacidad.tsx
-- en VIGIIAP) para que un despliegue nuevo sin fila en `configuracion` refleje
-- lo mismo desde el primer momento.
